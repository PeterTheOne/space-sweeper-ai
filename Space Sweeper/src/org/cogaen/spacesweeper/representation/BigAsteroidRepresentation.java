...
public class BigAsteroidRepresentation extends BaseRepresentation {

	private static final double SHADOW_DISTANCE = 0.2;
	private SceneNode node1;
	private SceneNode node2;
	private SpriteVisual mark;
	
	public BigAsteroidRepresentation(Core core, CogaenId entityId) {
		super(core, entityId);
	}
	...
	public void engage() {
		...
		SpriteVisual visual = (SpriteVisual) ResourceService.getInstance(getCore()).getResource("BigAsteroidSpr");
		this.node1.addVisual(visual);
		this.mark = visual;
		
		visual = visual.newInstance();
		visual.getColor().setColor(new Color(0, 0, 0, 0.5));
...
//		ps.setActive(0.1);
		...
		ParticleSystem ps = ParticleSystemService.getInstance(getCore()).getFromPool(PlayView.SMOKE_POOL);
		ps.getEmitter().setPose(getNode().getPosX(), getNode().getPosY(), 0);
		ps.setActive(0.2);
		
		SoundService sndSrv = SoundService.getInstance(getCore());
		sndSrv.playFromPool(PlayView.SOUND_BIG_EXPLOSION_POOL);
	}
